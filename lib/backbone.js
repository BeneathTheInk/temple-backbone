(function ( global, factory ) {
	...
	var trackModel = Temple.Backbone.trackModel = function(obj) {
		...
		var getDependency = obj.getDependency = function(attr) {
			var dep = deps[attr];
			if (dep == null) dep = deps[attr] = new Temple.Dependency();
			return dep;
		}

		var depend = obj.depend = function(k) {
			var attrs = Array.prototype.slice.call(arguments, 0);

			if (!attrs.length) mainDep.depend();
			else attrs.forEach(function(attr) {
				if (Array.isArray(attr)) return depend.apply(null, attr);
				getDependency(attr).depend();
			});
		...
		obj.get = function(k, options) {
			if (options == null || options.depend !== false) depend(k);
			return getter.apply(this, arguments);
		}

		var toJSON = obj.toJSON;
		obj.toJSON = function(options) {
			// depend on any change made to the collection
			if (options == null || options.depend !== false) depend();
